...
每个新写作任务先由用户选择 `快速 / 标准 / 深度`。选择后才进入对应的需求收集与写作流程，避免 Agent 擅自把一篇文章当成小修改，或把一句话润色拖进冗长流程。
...
- 在 Codex、Claude Code、OpenClaw、Hermes、Cursor、GitHub Copilot、Gemini CLI 及其他 Agent Skills 客户端中使用；文件缺失时可检查、修复或降级继续工作。
...
完整包遵循 [Agent Skills](https://agentskills.io/specification) 开放格式。权威副本是：
...
```text
skills/writing-craft/SKILL.md
```

它用于 OpenClaw、Hermes 和其他遵循 Agent Skills 标准的客户端；Claude Code 使用 `.claude/skills/writing-craft/SKILL.md`，Codex 保持使用 `.agents/skills/writing-craft/SKILL.md`。
...
Hermes 的具体触发命令会随其版本与安装方式变化；将 `skills/writing-craft/` 放入 Hermes 配置的 skills root 后，可使用该版本提供的 Skill 入口，或直接要求 Agent 读取其中的 `SKILL.md`。

如果平台没有自动发现 skill，直接让 Agent 读取对应平台目录下的 `SKILL.md` 即可。复制、恢复或切换分支后，建议从项目根目录重开 Agent 会话。完整兼容矩阵与安装回退见 [Agent 兼容说明](references/agent-compatibility.md)。
...
├── skills/writing-craft/             # Agent Skills 标准镜像，供 OpenClaw、Hermes 等使用
...
│   ├── agent-compatibility.md        # 跨 Agent 安装与发现路径
...
- `--sync` 用权威 `.agents/skills/writing-craft/` 副本刷新根目录、Claude 镜像和标准 `skills/` 镜像。
...
独立安装的 `.agents/skills/writing-craft/` 或 `skills/writing-craft/` 文件夹不会被误认为损坏的完整分发包，也不会在宿主项目中擅自创建配置文件。
...
测试覆盖：跨 Agent 镜像一致性、Agent Skills 标准 frontmatter、OpenClaw/Hermes 标准镜像、入口与 reference 路由、缺失文件恢复、镜像漂移保护、独立 skill 安装、完整分发包重建、用户资料缺失处理和改写示例库。
